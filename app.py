...
def run_grader(document_file, student_name, extra_text):
    ...
    privacy_text = (
        ...
        f"Relevant to assignment : {guard.get('is_relevant', '?')}\n"
        f"Method chosen          : {guard.get('method_chosen', '?')}\n"
        f"Has step-by-step       : {guard.get('has_steps', '?')}\n"
        f"Has justification      : {guard.get('has_justification', '?')}\n"
